...
class ClinicalStore:
    ...
    def list_patient_summaries(
        ...
        *,
        ...
        limit: int = 200,
        submitted_requests_only: bool = False,
    ) -> list[dict]:
        ...
        if submitted_requests_only:
            with engine.begin() as connection:
                request_rows = [
                    dict(row)
                    for row in connection.execute(
                        records.select()
                        .where(records.c.source_app.in_(("stroke", "emc")))
                        .where(
                            records.c.status.in_(
                                ("PENDING_REVIEW", "APPROVED", "APPROVED_FOR_ISSUE", "REJECTED")
                            )
                        )
                    ).mappings()
                ]
            request_patient_ids = {
                ...
                and not (
                    isinstance(row.get("output_payload"), dict)
                    and row["output_payload"].get(DEMO_SEED_MARKER_KEY) == DEMO_SEED_MARKER_VALUE
                )
            }
            if not request_patient_ids:
                return []
            patient_query = patient_query.where(patients.c.id.in_(request_patient_ids))
